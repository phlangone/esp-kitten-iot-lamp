...
// Estrutura para armazenar o frame de dados
typedef struct {
  struct {
...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
    ...
    case ESP_SPP_DATA_IND_EVT:
        ...
        frame.control_byte.color_mode = (param->data_ind.data[0] & 0x01);  
        frame.control_byte.on_off = (param->data_ind.data[0] >> 1) & 0x01;  

        frame.red_byte = param->data_ind.data[1];  
        frame.green_byte = param->data_ind.data[2]; 
        frame.blue_byte = param->data_ind.data[3];   

        if(DEBUG)
        {
            ESP_LOGI(SPP_TAG, "Color mode: %d", frame.control_byte.color_mode);
            ESP_LOGI(SPP_TAG, "On/Off: %d", frame.control_byte.on_off);
            ESP_LOGI(SPP_TAG, "Red: %d, Green: %d, Blue: %d", frame.red_byte, frame.green_byte, frame.blue_byte);
        }

        if(frame.control_byte.on_off == 1)
        {
            led_strip_fill(frame.color_data);
        }
        else
        {
            led_strip_fill(0);
